...
def join_log_files(name: str):
    ...
    """
    with open(f"joined/{name}.log", "w", encoding="utf-8") as f2_:
        for subdir in ["2", "4", "6"]:
            if os.path.exists(f"data/{subdir}/{name}_additional.log"):
                with open(
                    f"data/{subdir}/{name}_additional.log", "r", encoding="utf-8"
                ) as f:
                    for line in f:
                        if "NAME" not in line:
                            f2_.write(line)
...
if __name__ == "__main__":
    ...
    for _, row in df.iterrows():
        ...
        print(row[0])
        with open(f"final/{row[0]}.log", "w", encoding="utf-8") as f2:
